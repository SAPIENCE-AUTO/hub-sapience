...
export default createEndpoint({
  ...
  inputSchema: z.object({ slug: z.string(), observerId: z.string(), body: z.string().min(1).max(2000) }),
  ...
  execute: async ({ input }) => {
    ...
    const inserted = await pool.query(
      `insert into observation_chat (session_id, observer_id, body, es_productor)
       values ($1, $2, $3, false) returning id, created_at`,
      [row.session_id, input.observerId, input.body],
    );
    const message = inserted.rows[0];
    await publishEvent(`observation:${row.session_id}`, 'chat.message', {
      id: message.id, nombre: row.nombre, esProductor: false, body: input.body, createdAt: message.created_at,
    });
