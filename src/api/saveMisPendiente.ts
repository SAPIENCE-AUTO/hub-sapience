...
export default createEndpoint({
  ...
  execute: async ({ input, context }) => {
    ...
    const { rows } = await pool.query(
      `insert into pendientes_personales (user_id, titulo, notas, area, status, fuente, fecha_limite)
       values ($1, $2, $3, $4, 'Pendiente', 'manual', $5)
       returning id`,
      [userId, input.titulo.trim(), input.notas || null, input.area || 'Sin clasificar', input.fechaLimite || null],
    );
    return { id: rows[0].id };
  },
});
